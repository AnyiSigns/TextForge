...
test.describe('基础页面与认证', () => {
  ...
  test('未登录受保护页面重定向到登录页', async ({ page }) => {
    const routes = ['/zh/', '/zh/projects', '/zh/projects/new', '/zh/characters', '/zh/characters/create', '/zh/settings', '/zh/knowledge', '/zh/tasks', '/zh/assets', '/zh/api-keys'];
...
test.describe('登录与项目管理', () => {
  ...
  test('创建新项目', async ({ page }) => {
    ...
    await page.click('button[type="submit"]');
    await page.waitForURL((url) => url.pathname.startsWith('/zh/projects/') && !url.pathname.includes('/projects/new'));
  ...
  test('项目详情与生成', async ({ page }) => {
    ...
    await page.goto(`/zh/projects/${createdProjectId}`);
  ...
  test('删除测试项目', async ({ page }) => {
    ...
    const card = page.locator(`a[href="/zh/projects/${createdProjectId}"]`).locator('xpath=ancestor::*[@data-slot="card"]');
...
test.describe('角色管理', () => {
  ...
  test('删除测试角色', async ({ page }) => {
    ...
    await page.goto('/zh/characters');
    await page.once('dialog', (d: any) => d.accept());
    const card = page.locator(`a[href="/zh/characters/${createdCharacterId}/chat"]`).locator('xpath=ancestor::*[@data-slot="card"]');
    await card.locator('button:has(.lucide-trash-2)').click();
    await page.waitForTimeout(300);
