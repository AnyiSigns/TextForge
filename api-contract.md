...
| 状态码 | 含义 | 前端行为 | 响应体格式 |
|---|---|---|---|
| `200` | 成功 | 正常解析 | 见 §1.7 两种风格 |
| `201` | 创建成功 | 同 200（项目创建可能 201 或 200，前端都兼容） | 同 200 |
| `202` | 任务已受理（异步） | 同 200，之后轮询结果接口 | 同 200 |
| `204` | 删除成功 | 无 body | 无 body |
| `400` | 参数错误 | 弹错误提示 | `{ "message": "..." }` 或 `{ "error": "..." }` |
| `401` | 未登录 / token 失效 | 自动 refresh，失败则跳登录 | `{ "message": "未登录" }`（不要返回 HTML） |
| `404` | 资源不存在 / 探活离线 | 探活接口用 404 表示「后端离线」 | `{ "message": "..." }` 或空 |
| `412` | 乐观锁版本冲突（If-Match 不匹配） | 提示「数据已被他人修改」 | `{ "message": "版本冲突" }` |
| `429` | 限流 | 自动重试 | `{ "message": "请求过于频繁" }` |
| `500+` | 服务端错误 | 自动重试 / 报错 | `{ "message": "服务器错误" }` 或 `{ "error": "..." }` |

### 1.7 通用响应格式

后端按接口类型分两种响应风格，前端按约定解析；**同一端点不要混用**。

#### 风格一：资源包装式（列表 / 详情 / 创建）

列表/详情/创建类接口使用「单数键包数据」结构：
...
#### 风格二：直接体（简单动作 / 认证 / 生成任务）

「操作成功/失败」类接口直接返回结果体，不额外包装：

```jsonc
// 登录
{ "access_token": "eyJhbGciOi...", "refresh_token": "rt-xxxxx", "user": { ... } }
...
// 生成任务
{ "task": { "id": "task_1", "kind": "image", "status": "pending", ... } }
```
