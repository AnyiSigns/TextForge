...
function roleLabel(char: Character | { role?: string; roleType?: string }): string | null {
  const role = char.role ?? char.roleType;
  if (!role) return null;
  if (role === 'custom') return null;
  return characterRoleLabel(role as CharacterRole) ?? role;
}

export function ProjectCharactersTab({ projectId }: { projectId: number }) {
  const {
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          <Link href={`/characters/create?bookId=${projectId}`}>
        ...
        <div className="space-y-2 stagger">
          {filtered.map((char) => (
            <CharacterCard
              key={char.id}
              char={char}
              isEditing={editingId === String(char.id)}
              editName={editName}
              editDesc={editDesc}
              editRole={editRole}
              viewMode="list"
