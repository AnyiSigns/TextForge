...
interface CharacterRelationsSheetProps {
  ...
  relDraft: CharacterRelation[];
  projectChars: { id: number; name: string }[];
  onTargetChange: (id: string, patch: { target?: string; relation?: string }) => void;
              ...
              {relDraft.map((r) => (
                <div key={r.id} className="space-y-2 rounded-xl border border-border/40 p-3">
                  <div className="flex items-center gap-2">
                     <select
                       value={r.target}
                       onChange={(e) => onTargetChange(r.id, { target: e.target.value })}
