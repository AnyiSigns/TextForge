...
interface CharacterDetailSheetProps {
  ...
  roleLabel: (char: Character | { role?: string; roleType?: string }) => string | null;
...
export function CharacterDetailSheet(props: CharacterDetailSheetProps) {
  const {
    detailChar, detailRole, charNameById, rolePresets, roleLabel, statusBadge,
    onOpenStatus, onDetailRole, onSaveDetailRole,
            ...
            <SheetHeader className="px-5 pt-5 pb-4 border-b border-border/30">
              <SheetTitle className="flex items-center gap-3 text-xl">
                <Avatar className="w-11 h-11 ring-1 ring-border/40">
                  <AvatarImage src={detailChar.avatarUrl} />
            ...
            <div className="mt-5 px-5 space-y-5">
              ...
              <div className="space-y-2">
                <p className="text-[11px] font-semibold text-muted-foreground uppercase tracking-[0.12em] flex items-center gap-1.5">
                  <Link2 className="w-3.5 h-3.5" /> 角色关系（{detailChar.relationshipChain?.length ?? 0}）
                </p>
                {detailChar.relationshipChain && detailChar.relationshipChain.length > 0 ? (
                  <div className="space-y-1.5">
                    {detailChar.relationshipChain.map((r) => (
                      <div key={r.id} className="flex items-center gap-2 text-sm">
                        <span className="font-medium">{charNameById(r.target)}</span>
