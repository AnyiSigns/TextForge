        ...
        {tab === 'videos' && (
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-6 items-start">
            <GenerationForm
              ...
              kind="video"
              defaultBookId={activeProjectId}
              defaultChapterId={defaultChapterId}
              useCase={deepChapterId ? 'chapter_anim' : undefined}
              steps={projectSteps}
              chapterOptions={chapterOptions}
               characters={activeProjectId ? characters.filter((c) => c.bookId === Number(activeProjectId)).map((c) => ({ id: String(c.id), name: c.name, referenceImages: c.referenceImages, referenceImage: c.referenceImage })) : []}
               projectCharacters={activeProjectId ? characters.filter((c) => c.bookId === Number(activeProjectId)).map((c) => ({ id: String(c.id), name: c.name, referenceImages: c.referenceImages, referenceImage: c.referenceImage })) : []}
              context={genContext}
