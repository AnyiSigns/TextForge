...
export async function getManuscriptChapters(bookId: number): Promise<ManuscriptChapter[]> {
  const db = await getDB();
...
export async function deleteManuscriptByBook(bookId: number): Promise<void> {
  const db = await getDB();
  const all = (await db.getAll(MS_STORE)) as ManuscriptChapter[];
