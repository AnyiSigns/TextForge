    ...
    <div className="h-full overflow-y-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <WorkflowIcon size={22} className="text-muted-foreground" />
          <div>
            <h1 className="text-lg font-semibold">创作流程</h1>
            <p className="text-xs text-muted-foreground">管理角色节点和工作流，控制 Agent 生成内容的流程</p>
          </div>
        </div>
          ...
          {filtered.map((wf) => (
            view === 'grid' ? (
              ...
              <Link key={wf.id} href={`/workflow/${wf.id}`}
                className="flex items-center justify-between p-3 rounded-lg border border-border/40 bg-card hover:bg-card/60 no-underline text-foreground group">
