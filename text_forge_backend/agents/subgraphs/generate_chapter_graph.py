...
async def think_node(state: GenerateChapterState) -> Dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        SystemMessage(content="你是写作规划助手。分析用户指令和书籍上下文，为章节写作制定策略。"),
...
async def plan_node(state: GenerateChapterState) -> Dict[str, Any]:
    ...
    llm = ModelFactory(state["model_config"])
    prompt = ChatPromptTemplate.from_messages([
        SystemMessage(content="你是写作规划助手。根据分析结果，制定详细的章节写作大纲。"),
...
async def execute_node(state: GenerateChapterState) -> Dict[str, Any]:
    ...
    llm = ModelFactory(state["model_config"])
    prompt = ChatPromptTemplate.from_messages([
        SystemMessage(content="你是专业的小说写作助手。根据大纲和上下文，写出高质量的章节正文。"),
        HumanMessage(content=f"""书籍上下文：
{state.get('context', '')}
...
async def reflect_node(state: GenerateChapterState) -> Dict[str, Any]:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        HumanMessage(content=f"""请审阅以下章节正文，指出需要改进的地方（逻辑、文笔、一致性等）。
...
请直接输出审阅意见："""),
    ...
    return {"reflection": reflection, "step_outputs": {**state.get("step_outputs", {}), "reflect": reflection}}


def _route_after_reflect(state: GenerateChapterState) -> str:
    reflection = state.get("reflection", "")
    if reflection and ("严重" in reflection or "不一致" in reflection or "逻辑错误" in reflection):
        return "execute"
    return END
...
def build_generate_chapter_graph():
    ...
    builder.add_conditional_edges("reflect", _route_after_reflect, {"execute": "execute", END: END})
