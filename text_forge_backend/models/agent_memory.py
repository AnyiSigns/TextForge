...
class AgentMemory(Base):
    ...
    book_id: Mapped[int | None] = mapped_column(ForeignKey("books.id", ondelete="CASCADE"), nullable=True, index=True, comment="书籍ID")
    memory_type: Mapped[str] = mapped_column(String(20), nullable=False, index=True, comment="记忆类型")
    content: Mapped[str] = mapped_column(Text, nullable=False, comment="记忆内容")
    related_chapter_id: Mapped[int | None] = mapped_column(ForeignKey("chapters.id", ondelete="SET NULL"), nullable=True, comment="关联章节ID")
    ...
    embedding: Mapped[list[float] | None] = mapped_column(Vector(1536), nullable=True, comment="向量嵌入")
