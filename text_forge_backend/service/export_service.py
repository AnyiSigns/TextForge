...
class ExportService:
    ...
    async def export_book(self, user_id: int, book_id: int, fmt: str, include_outline: bool, include_characters: bool, volume_ids: Optional[List[int]]) -> Dict[str, Any]:
        ...
        for volume in volumes:
            chapters_stmt = select(Chapter).where(Chapter.volume_id == volume.id).order_by(Chapter.sort_order)
        ...
        if include_characters:
            chars_stmt = select(Character).where(Character.book_id == book_id).order_by(Character.id)
            chars_result = await self.session.execute(chars_stmt)
            characters = [{"name": c.name, "description": c.description or "", "role_type": c.role_type or ""} for c in chars_result.scalars().all()]
    ...
    async def _build_pdf(self, book, chapters, characters, outline):
        ...
        if characters:
            ...
            for c in characters:
                story.append(Paragraph(f"{c['name']} ({c.get('role_type','')})", styles["Heading3"]))
                if c.get("description"):
