...
class ChatService:
    ...
    async def get_conv(self, user_id: int, thread_id: str):
        ...
        if not conv:
            raise RuntimeError(
                f"无法获取或创建会话：user_id={user_id}, thread_id={thread_id}"
            )
        return conv
    ...
    async def add_msg(
        ...
        await self.msg_repo.add(
            role=role, content=content, think=think, conversation_id=conv.id
        )
        ...
        try:
            ...
            async for chunk, metadata in graph.astream(
                ...
                if isinstance(chunk, AIMessage):
                    ...
                    think_content = chunk.additional_kwargs.get(
                        "reasoning_content"
                    ) or chunk.response_metadata.get("reasoning_content")
                    if think_content:
