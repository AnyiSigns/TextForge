...
from domains.agent.subgraphs.generate_chapter_graph import (
    build_generate_chapter_graph,
    GenerateChapterState,
)
from cofig.logging import get_logger

logger = get_logger(__name__)


async def _get_previous_chapter_context(
    session: AsyncSession, book_id: int, chapter_id: int
) -> Dict[str, Any]:
...
def build_generate_chapter_tool(session_factory, model_config: Optional[dict] = None):
    ...
    async def generate_chapter(
        ...
        previous_context = await _get_previous_chapter_context(
            session, book_id, chapter_id
        )
        ...
        state: GenerateChapterState = {
            ...
            "step_outputs": {},
            "previous_chapter_summary": previous_context.get(
                "previous_chapter_summary"
            ),
            "previous_chapter_content": previous_context.get(
