...
async def save_memory(
    ...
    priority: int = 5,
    source: str = "agent_self_reflection",
...
async def list_memories_by_type(
    session: AsyncSession, user_id: int, memory_type: str, book_id: Optional[int] = None
) -> List[dict]:
    """List memories filtered by type."""
    service = await _get_service_from_context(session)
    return await service.list_memories(
        user_id=user_id, book_id=book_id, memory_type=memory_type
    )
...
async def update_memory(
    ...
    memory = await service.update_memory(
        user_id=user_id, memory_id=memory_id, data=payload
    )
    if not memory:
        return {"ok": False, "detail": "记忆不存在"}
