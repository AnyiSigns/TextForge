...
from domains.book.outline_repo import OutlineRepository
from domains.book.world_repo import WorldRepository
...
def _build_lookup_tools(session_factory):
    ...
    async def lookup_characters(
        user_id: int, book_id: int, names: Optional[List[str]] = None
    ) -> List[dict]:
        """lookup_characters tool."""
        session = await session_factory()
        characters = await CharacterRepository(session).book_character_detail(
    ...
    async def lookup_outline(
        book_id: int, chapter_id: Optional[int] = None
    ) -> List[dict]:
        """lookup_outline tool."""
        session = await session_factory()
    ...
    async def lookup_locations(book_id: int, query: Optional[str] = None) -> List[dict]:
        ...
        if query:
            locations = [
                ...
                if query in (loc.name or "") or query in (loc.description or "")
            ]
        return [
            {
                "id": loc.id,
    ...
    async def lookup_timeline(
        ...
        if query:
            events = [
                ev
                for ev in events
                if query in (ev.name or "") or query in (ev.description or "")
            ]
        return [
            {
                "id": ev.id,
    ...
    async def lookup_foreshadowing(
        book_id: int, status: str = "planted", query: Optional[str] = None
    ) -> List[dict]:
        """lookup_foreshadowing tool."""
        session = await session_factory()
        items = await WorldRepository(session).list_foreshadowings(
            book_id, status=status
        )
        if query:
    ...
    async def lookup_plot_threads(
        book_id: int, status: str = "active", query: Optional[str] = None
    ) -> List[dict]:
        ...
        if query:
            items = [
                item
                for item in items
                if query in (item.name or "") or query in (item.description or "")
            ]
        return [
            {
                "id": item.id,
...
def _build_agent_tools(session_factory, model_config: Optional[dict] = None):
    ...
    async def search_public_docs(
        book_id: int, query: str, target_book_id: Optional[int] = None, top_k: int = 5
    ) -> List[dict]:
    ...
    async def personal_rag_search(
        ...
        try:
            ...
            doc_stmt = sa_select(Document.id, Document.file_name).where(
                Document.user_id == user_id, Document.scope == "personal"
            )
    ...
    async def get_book_context(user_id: int, book_id: int) -> dict:
        ...
        return {
            ...
            "volumes": [
                {"id": v.id, "title": v.title, "summary": v.summary} for v in volumes
            ],
        }
    ...
    async def auto_extract_entities(user_id: int, book_id: int, content: str) -> dict:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                SystemMessage(
                    content="""你是实体提取助手。从给定文本中提取人物、地点、事件。
输出 JSON 格式：
{
...
只输出 JSON，不要其他内容。"""
    ...
    async def batch_create_entities(
        ...
        try:
            ...
            for item in timeline_events or []:
                ...
                created["timeline_events"].append(
    ...
    async def update_foreshadowing(item_id: int, book_id: int, data: dict) -> dict:
        ...
        session = await session_factory()
        instance = await WorldRepository(session).update_foreshadowing(
            item_id, book_id, data
        )
        if not instance:
        ...
        return {
            ...
            "resolved_at_chapter_id": instance.resolved_at_chapter_id,
        }
    ...
    async def update_plot_thread(item_id: int, book_id: int, data: dict) -> dict:
        ...
        session = await session_factory()
        instance = await WorldRepository(session).update_plot_thread(
            item_id, book_id, data
        )
        if not instance:
        ...
        return {
            ...
            "name": instance.name,
            "status": instance.status,
            "progress_note": instance.progress_note,
        }
    ...
    async def update_timeline(item_id: int, book_id: int, data: dict) -> dict:
        ...
        session = await session_factory()
        instance = await WorldRepository(session).update_timeline_event(
            item_id, book_id, data
        )
        if not instance:
    ...
    async def polish_text(text: str, instruction: str = "") -> dict:
        ...
        system = SystemMessage(
            content="你是专业的文字润色助手。改进文本的表达、节奏和可读性，保持原意不变。直接输出润色后的文本。"
        )
        human = HumanMessage(
    ...
    async def rewrite_paragraph(text: str, instruction: str = "") -> dict:
        ...
        system = SystemMessage(
            content="你是专业的改写助手。根据用户指令改写文本，保持核心含义但改变表达方式。直接输出改写后的文本。"
        )
        human = HumanMessage(
    ...
    async def expand_text(text: str, target_length: Optional[int] = None) -> dict:
        ...
        system = SystemMessage(
            content="你是专业的扩写助手。在保持原意和风格的基础上，丰富细节、描写和对话，使文本更加生动。直接输出扩写后的文本。"
        )
        human = HumanMessage(
            content=f"请扩写以下文本，目标字数约 {target} 字：\n{text[:4000]}"
        )
        try:
            result = await llm.main.ainvoke([system, human])
    ...
    async def summarize_selected(text: str, max_length: Optional[int] = None) -> dict:
        ...
        system = SystemMessage(
            content="你是专业的摘要助手。请简洁地总结文本内容，保留关键信息和核心情节。"
        )
        human = HumanMessage(
            content=f"请将以下文本总结为 {target} 字以内的摘要：\n{text[:6000]}"
        )
        try:
            result = await llm.main.ainvoke([system, human])
    ...
    async def suggest_alternatives(
        text: str, position: Optional[int] = None, count: int = 3
    ) -> dict:
        ...
        system = SystemMessage(
            content="你是写作建议助手。针对给定文本，提供多个不同风格的改写建议。"
        )
        human = HumanMessage(
            content=f"请提供 {count} 种不同风格的改写建议：\n{text[:4000]}"
        )
        try:
            result = await llm.main.ainvoke([system, human])
    ...
    async def check_consistency(book_id: int, chapter_id: Optional[int] = None) -> dict:
        ...
        session = await session_factory()
        from models.book import Book, Chapter, ChapterContent, Character
        from domains.book.repository import CharacterRepository
        from domains.book.world_repo import WorldRepository
        ...
        system = SystemMessage(
            content="你是 consistency 检查助手。检查正文中的人物、地点、时间线是否与设定一致。列出不一致的地方。"
        )
        human = HumanMessage(
    ...
    async def check_grammar(text: str) -> dict:
        ...
        system = SystemMessage(
            content="你是语法检查助手。检查文本中的语法、拼写和标点错误，列出问题并给出修正建议。"
        )
    ...
    async def search_across_books(
        ...
        try:
            ...
            doc_stmt = sa_select(
                Document.id, Document.file_name, Document.book_id
...
def build_tool_node(session_factory, model_config: Optional[dict] = None):
    ...
    tools.append(
        build_generate_chapter_tool(session_factory, model_config=model_config)
    )
