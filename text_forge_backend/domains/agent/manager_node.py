...
from domains.agent.state import ParentState, ToolState, MainState, AuditState
from langgraph.graph import END
from domains.book.context_config_repo import BookContextConfigRepository
from shared.database import db_manager
from domains.agent.tool_node import tool_node
from domains.agent.main_node import main_node
from domains.agent.audit_node import audit_node
...
async def call_main(state: ParentState) -> dict:
    ...
    Returns:
        主模型执行结果。
    """
    from domains.agent.state import MainState
    from domains.agent.graphs.registry import graph_register
...
async def call_compression(state: ParentState) -> dict:
    ...
    Returns:
        压缩结果。
    """
    from domains.agent.state import AuditState
    from domains.agent.graphs.registry import graph_register
