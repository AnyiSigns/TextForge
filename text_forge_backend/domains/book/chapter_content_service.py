...
from domains.book.chapter_content_repo import ChapterContentRepository
...
class ChapterContentService:
    ...
    async def list_contents(self, chapter_id: int):
        try:
            ...
            raise AppException(
                status_code=500,
                detail="获取章节正文列表失败",
                error_code="LIST_CONTENTS_FAILED",
            )
    ...
    async def get_latest_content(self, chapter_id: int):
        try:
            ...
            if not result:
                raise AppException(
                    status_code=404, detail="正文不存在", error_code="CONTENT_NOT_FOUND"
                )
            return result
        except AppException:
            raise
            ...
            raise AppException(
                status_code=500,
                detail="获取最新正文失败",
                error_code="GET_LATEST_CONTENT_FAILED",
            )
    ...
    async def create_content(self, chapter_id: int, content: str):
        try:
            ...
            raise AppException(
                status_code=500,
                detail="创建正文失败",
                error_code="CREATE_CONTENT_FAILED",
            )
    ...
    async def get_content_by_version(self, chapter_id: int, version: int):
        try:
            ...
            raise AppException(
                status_code=500,
                detail="获取指定版本正文失败",
                error_code="GET_CONTENT_BY_VERSION_FAILED",
            )

    async def diff_versions(self, chapter_id: int, from_version: int, to_version: int):
        try:
            from_item = await self.content_repo.get_content_by_version(
                chapter_id, from_version
            )
            to_item = await self.content_repo.get_content_by_version(
                chapter_id, to_version
            )
            ...
            return {
                ...
                "from_created_at": (
                    from_item.created_at.isoformat() if from_item.created_at else None
                ),
                "to_created_at": (
                    to_item.created_at.isoformat() if to_item.created_at else None
                ),
            }
        except AppException:
            raise
            ...
            raise AppException(
                status_code=500,
                detail="获取版本差异失败",
                error_code="DIFF_VERSIONS_FAILED",
            )
