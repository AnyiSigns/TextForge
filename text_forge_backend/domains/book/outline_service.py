...
class OutlineService:
    ...
    async def list_outlines(self, book_id: int):
        ...
        try:
            ...
            raise AppException(
                status_code=500,
                detail="获取大纲列表失败",
                error_code="LIST_OUTLINES_FAILED",
            )
    ...
    async def get_outline(self, book_id: int, outline_id: int):
        ...
        try:
            ...
            if not result:
                raise AppException(
                    status_code=404, detail="大纲不存在", error_code="OUTLINE_NOT_FOUND"
                )
            return result
        except AppException:
            raise
            ...
            raise AppException(
                status_code=500, detail="获取大纲失败", error_code="GET_OUTLINE_FAILED"
            )
    ...
    async def update_outline(self, outline_id: int, **data):
        ...
        try:
            ...
            raise AppException(
                status_code=500,
                detail="更新大纲失败",
                error_code="UPDATE_OUTLINE_FAILED",
            )
    ...
    async def delete_outline(self, outline_id: int):
        ...
        try:
            ...
            raise AppException(
                status_code=500,
                detail="删除大纲失败",
                error_code="DELETE_OUTLINE_FAILED",
            )

    async def auto_summarize_if_needed(
        self, outline_id: int, book_id: int, user_id: int, data: dict
    ):
        ...
        try:
            ...
            model_conf = await ModelConfRepository(self.session).query_user_model(
                user_id
            )
            if not model_conf:
                return
            ...
            res = await llm.main.ainvoke(
                [SystemMessage("你是章节摘要助手"), HumanMessage(prompt)]
            )
