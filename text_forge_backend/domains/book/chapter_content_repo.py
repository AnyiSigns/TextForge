...
class ChapterContentRepository(BaseRepository[ChapterContent]):
    ...
    async def get_content_by_version(self, chapter_id: int, version: int):
        ...
        Returns:
            章节内容实例，不存在返回 None。
        """
        stmt = select(ChapterContent).where(
            ChapterContent.chapter_id == chapter_id, ChapterContent.version == version
        )
        result = await self.session.execute(stmt)
        return result.scalar_one_or_none()
    ...
    async def _next_version(self, chapter_id: int) -> int:
        ...
        Returns:
            下一个版本号。
        """
        stmt = select(func.coalesce(func.max(ChapterContent.version), 0)).where(
