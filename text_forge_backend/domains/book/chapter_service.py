...
from domains.book.chapter_repo import ChapterRepository
...
class ChapterService:
    ...
    async def list_chapters(self, volume_id: int):
        try:
            ...
            raise AppException(
                status_code=500,
                detail="获取章列表失败",
                error_code="LIST_CHAPTERS_FAILED",
            )
    ...
    async def get_chapter(self, volume_id: int, chapter_id: int):
        try:
            ...
            if not result:
                raise AppException(
                    status_code=404, detail="章节不存在", error_code="CHAPTER_NOT_FOUND"
                )
            return result
        except AppException:
            raise
            ...
            raise AppException(
                status_code=500, detail="获取章失败", error_code="GET_CHAPTER_FAILED"
            )
    ...
    async def create_chapter(self, volume_id: int, **data):
        try:
            ...
            raise AppException(
                status_code=500, detail="创建章失败", error_code="CREATE_CHAPTER_FAILED"
            )
    ...
    async def update_chapter(self, chapter_id: int, **data):
        try:
            ...
            raise AppException(
                status_code=500, detail="更新章失败", error_code="UPDATE_CHAPTER_FAILED"
            )
    ...
    async def delete_chapter(self, chapter_id: int):
        try:
            ...
            raise AppException(
                status_code=500, detail="删除章失败", error_code="DELETE_CHAPTER_FAILED"
            )
