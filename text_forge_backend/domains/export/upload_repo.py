...
async def create_document(
    session: AsyncSession, file: UploadFile, md5: str, content: bytes
) -> Document:
    """创建文档记录。
...
async def create_chunks(
    session: AsyncSession, doc_id: int, chunks: List[str], embedder
) -> List[Chunk]:
...
async def process_upload(
    session: AsyncSession, file: UploadFile, emb_config: dict | None = None
):
    """处理文档上传全流程。
